...
#align(center + top)[
  ...
  #v(0.5cm)
  #text(size: 17pt, fill: rgb("#475569"))[实验报告]
  #v(1.1cm)
  #block(
    width: 72%,
    inset: (x: 24pt, y: 20pt),
...
与通用递归 DNS（如 BIND、Unbound）不同，本课题刻意将功能收敛为「策略驱动的转发网关」：不实现自根向下的全球递归迭代，不维护 TTL 感知缓存，不处理 TCP 53 fallback，主要面向 A 记录（QTYPE=1）的 UDP 查询。这一约束使实现复杂度可控，却仍能覆盖网络协议课设的核心能力——二进制报文编解码、网络字节序、事件驱动 I/O、应用层错误语义（RCODE）与真实客户端（`nslookup`、`dig`）互操作。

实现层面，我们独立封装协议模块处理首部位域与 `htons`/`ntohs` 边界转换；在域名解码中引入指针跳转计数与偏移合法性校验以抵御畸形报文；通过 ID 映射表与上游 ID 替换避免多客户端 Transaction ID 冲突；主循环以 `select()` 10ms 超时实现事件驱动；上游中继 3 秒超时并在失败时返回 SERVFAIL（fix-B）。
...
*第四，多客户端语义下的 ID 与会话管理。* UDP 无连接，Transaction ID 是客户端匹配响应的唯一线索；多主机并发时 ID 空间可能碰撞。中继转发须分配新 ID 并记录 `(original_id, client_addr, new_id)`；回包时还原 ID。当前同步模型在 `relay_to_upstream` 阻塞期间无法处理新查询，但 ID 表与 5 秒老化逻辑已为异步 epoll 模型预埋数据结构。
...
#figure(
  ...
  table(
    ...
    [主控层], [`main.c`], [`select` 主循环；`relay_to_upstream` 上游中继；环境变量与 bind 逻辑], [`main.c`],
  ),
  caption: [模块划分与主要接口一览],
)
...
*上游中继（Upstream Relay）*：`config_lookup` 返回 NULL 时，域名不在本地策略中，须透明转发至上游 `114.114.114.114:53`。转发路径：`relay_to_upstream` 创建临时 UDP socket、设置 3s 接收超时、替换 ID、`add_record` 登记映射、`sendto` 上游、`recvfrom` 等待、还原 ID、`sendto` 回客户端。成功时响应可能含多条 A 记录及 EDNS OPT 伪节（公网 DNS 常见），本实现原样透传字节流，不改变 TTL 与 Additional 结构。失败（超时、send/recv 错误、映射表满）时由主循环构造 SERVFAIL（fix-B），使客户端在数秒内失败而非长时间阻塞。
...
#figure(
  ...
  table(
    ...
    [*成员*], [*学号*], [*主要负责*],
    [张恒基], [2024210926], [
      主控层 `main.c`：`socket/bind`、环境变量端口、`select` 事件循环、查表三路调度、`relay_to_upstream` 同步中继与 fix-B、集成联调与性能边界分析
    ],
    [尹浩铭], [2024210910], [
...
+ *上游中继*：`relay_to_upstream` 独立临时 socket、换 ID 并 `add_record`、`SO_RCVTIMEO=3s`、响应原样透传（步骤 5/13）
...
#figure(
  ...
  table(
    ...
    inset: 7pt,
    [*成员*], [*文件*], [*核心函数*], [*第四章验收对应*],
    [张恒基], [`main.c`], [`main` `select` `relay_to_upstream` `config_lookup` 分支], [步骤 2 启动/bind；4/6/12 拦截调度；5/13 中继；9 fix-A 分支；14 fix-B；主循环流程图对照],
...
#diagtext(
  ...
  [图 4 系统总体架构],
  [
    端到端数据流：UDP 查询 → `select` 可读 → `recvfrom` → `dns_parse_query` → `config_lookup` → 三路分支 → `sendto` 响应。
    中继位于 Stub Resolver 与上游递归 DNS 之间：对客户端是「唯一 DNS 服务器」，对上游是「普通查询客户端」。
    单线程避免对 `g_config`、`id_map` 加锁；代价是同步 `relay_to_upstream` 阻塞主循环。启动链：`socket` → `bind` → `config_load` → `select` 主循环。
  ],
)
...
#screentext-wide(
  "服务启动与 208 条配置",
  "docs/screenshots/terminal-02-startup.png",
  [
...
事件驱动核心为 `select()`：将监听 fd 加入 `fd_set`，`timeval` 设为 10ms。返回 0 时无数据，进程在内核中短暂睡眠，CPU 占用近 0%；返回大于 0 时处理可读事件。与「循环 `recvfrom`」相比，这是本课设强调的*资源效率*要求。上游中继另建临时 UDP socket，并设 `SO_RCVTIMEO=3s`，避免主监听 socket 被长阻塞拖死——超时由 `relay_to_upstream` 内部处理，失败后再由主循环组 SERVFAIL。
...
*ID 映射层*（`id_map.h` / `id_map.c`）服务于上游中继场景，提供 `add_record`、`clear_timeout_records` 等接口，记录 original_id、new_id 与客户端四元组。当前同步模型下主要承担记录与老化；异步化后将成为响应路由核心。

*主控层*（`main.c`）是程序入口与调度中心：解析环境变量、创建并绑定 UDP socket、加载配置、运行 `select` 主循环、根据查表结果调用协议层组包或 `relay_to_upstream`，并处理 fix-A/fix-B 分支。主控层不直接解析域名标签字节，降低与协议细节的耦合。
...
#figure(
  ...
  table(
    ...
    [未命中], [—], [任意], [0/2], [`relay_to_upstream`；失败时 RCODE=2 SERVFAIL],
  ),
...
#screentext-wide(
  ...
  [
    ...
    建议打开 `src/main.c` 搜索 `config_lookup`，沿分支向下追踪至 `dns_build_a_response`、`dns_build_error_response`、`relay_to_upstream` 三处调用，再翻到 §4.3 对应步骤截图核对 RCODE。
  ],
)
...
*分发阶段（主循环流程图 dispatch 框内三路分支）*：`config_lookup(qname)` 以大小写不敏感方式查表。命中且 `ip.s_addr==0` → 本地拦截，构造 NXDOMAIN；命中且 IP 非零且 `qtype==DNS_QTYPE_A` → `dns_build_a_response` 填入 A 记录；命中且 IP 非零但 QTYPE 非 A → fix-A，空 NOERROR；未命中 → `relay_to_upstream`，函数内完成上游收发并在成功时直接 `sendto` 客户端；若 `relay_ret!=0` → fix-B，主循环构造 SERVFAIL。所有分支在 `sendto` 完成后回到 `select` 起点，形成无限服务循环。

本地拦截不向公网产生任何流量，网关侧可审计「策略拒绝」而不泄露用户查询到外部 resolver；本地解析使实验域名无需搭建真实 BIND 区文件即可演示 A 记录应答；上游中继对客户端完全透明，仅增加一跳 RTT。同步阻塞模型的排队效应可形式化理解为：若上游平均耗时 \(T_u\)，到达率为 \(\lambda\)，单线程下查询完成时间近似随负载线性增长——课程演示（单用户 `nslookup`）几乎无感知，而压测（`dnsperf` 高并发）会出现 Timeout 与 SERVFAIL 上升，这属于架构预期而非单纯 bug。
...
本组 ID 映射表固定容量 1024 条（`ID_MAP_SIZE`），采用环形槽位分配：维护全局写指针，依次寻找下一个空闲槽位插入；冲突时线性探测下一槽位。每条记录含 `original_id`、`new_id`、`client_ip`、`client_port`、`created_at` 时间戳。主循环每次收包前调用 `clear_timeout_records(now, 5)` 清理 5 秒前记录，防止表无限增长。`add_record` 失败（表满）时先 `clear_timeout_records(now, 0)` 强制清空全部记录再重试；仍失败则 `relay_to_upstream` 返回 −1，主循环向客户端返回 SERVFAIL。

当前同步实现中，响应回传主要依赖 `relay_to_upstream` 调用栈中的 `client_addr` 参数，`find_record_by_new_id` 尚未在 `main.c` 中用于异步回包；映射表承担记录与老化职责，并为将来 epoll 异步化预留数据结构。
...
#figure(
  ...
  block(width: 100%, breakable: true)[
    #block(
      ...
      #image("docs/screenshots/terminal-11-dig-bupt.png", width: 100%, fit: "contain")
    ]
  ],
...
`relay_to_upstream` 为同步实现：创建临时 UDP socket，设置 3 秒 `SO_RCVTIMEO`，将查询 ID 替换为新 ID 并 `add_record` 记入映射表，向 `114.114.114.114:53` 发送查询；`recvfrom` 收到响应后将 ID 还原为客户端原始值，再 `sendto` 回客户端。失败时主循环返回 SERVFAIL（fix-B）。ID 映射表（1024 条、5 秒超时清理）为异步扩展预留；当前同步模型下响应路由主要依赖调用栈中的 `client_addr`。

#screentext-wide(
  "dig baidu.com 上游中继",
  "docs/screenshots/terminal-13-dig-relay.png",
  [
    `dig` 应答含多条 IN A 记录，`flags: qr rd ra` 表明经上游递归返回。EDNS OPT 段来自公网 DNS，说明查询已穿透 `relay_to_upstream` 并成功回传至本机客户端。
  ],
)

该设计的优点是逻辑线性、易调试；缺点是上游 RTT 期间主循环无法 `select` 新查询。

#codefile("src/main.c — relay_to_upstream")[
```c
#define UPSTREAM_DNS_IP "114.114.114.114"
tv.tv_sec = 3;  /* SO_RCVTIMEO */
*(uint16_t *)modified_query = htons(new_id);
sendto(upstream_fd, modified_query, ...);
resp_len = recvfrom(upstream_fd, response, ...);
*(uint16_t *)response = htons(original_id);
sendto(sockfd, response, ..., client_addr);
```
]
...
#screentext-wide(
  "make clean && make",
  "docs/screenshots/terminal-01-build.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-03-nslookup-bupt.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-04-nslookup-008.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-05-nslookup-baidu.png",
  [
    返回多条公网 A 记录 → `relay_to_upstream` 成功、ID 已还原、响应透传。课程「上游中继」最直观验收现象。
  ],
)
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-06-nslookup-test0.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-07-nslookup-test1.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-08-nslookup-sina.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-09-nslookup-mx.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-10-dns-query.png",
  [
...
#screentext-wide(
  "dig bupt +comments",
  "docs/screenshots/terminal-11-dig-bupt.png",
  [
...
#screentext-wide(
  "dig 008.cn +comments",
  "docs/screenshots/terminal-12-dig-block.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-14-fix-b.png",
  [
...
+ *压力模式*（`bash scripts/run_dnsperf.sh stress`）：含 `baidu.com` 中继，`-c 100 -q 500`，暴露单线程同步 `relay_to_upstream` 瓶颈（高 Timeout、Queries lost）。
...
#figure(
  ...
  table(
    ...
    [stress], [`-c 100 -q 500` 含中继], [约 14%], [约 14], [同步上游阻塞致大量 Timeout],
  ),
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-15-dnsperf-light.png",
  [
...
#screentext-wide(
  ...
  "docs/screenshots/terminal-16-dnsperf-stress.png",
  [
    `Queries lost: 85.95%`，大量 `[Timeout]`（截图中已省略重复行）。根因：`relay_to_upstream` 同步阻塞主循环，100 客户端 × 500 未完成查询堆积，与架构分析一致——*非功能 bug*，而是单线程模型的预期表现。
...
张恒基、尹浩铭、林旭东三人完成 14 步自动化验证：编译零警告、配置 208 条、课程三必测（bupt / 008.cn / baidu.com）、`test0`/`test1`、fix-A、fix-B（iptables + SERVFAIL）及 `dig`/`dns_query.py` 协议对照全部通过。每一步终端截图与 `docs/verification/03-full-verification.log` 可交叉审计，满足课程「现象可复现、结论可验证」要求。已知局限：单线程同步中继在高并发压测下会出现排队超时，不属于功能验收范围，但提示生产环境须异步化与缓存。
...
*fix-B：上游挂起*：上游不可达时 `recvfrom` 无超时，主循环阻塞，客户端长时间无响应。解决：上游临时 socket 设置 `SO_RCVTIMEO` 为 3 秒；`relay_to_upstream` 失败时主循环返回 SERVFAIL，客户端可及时失败而非 indefinite 等待。
...
#keybox(
  [后续可扩展方向（超出本课设但具研究价值）],
  [
    *性能*：epoll 异步上游 + ID 表路由；最小 TTL 正向/负缓存降低上游 QPS。
    *协议*：TCP 53 fallback、EDNS0 4096 字节、AAAA/CNAME 类型分支。
    *安全*：响应速率限制、随机化 ID、丢弃畸形报文统计、DNS over TLS 上游。
    *运维*：热加载 `dnsrelay.txt`、结构化日志、Prometheus 查询计数器。
  ],
)

上述方向不影响本课程当前验收，但为将本中继原型演进为实验楼网关或研究用 DNS 策略平台提供了路线图。
...
#figure(
  ...
  block(width: 100%, breakable: true)[
    ...
    #table(
      ...
      [main.c], [入口、`select` 主循环、`relay_to_upstream`],
