...
# Run in WSL: bash platform/linux/verify/run_verification.sh
# Windows: platform/windows/verify/run_verification.ps1
set -euo pipefail
...

{
  ...
  echo "$ python3 platform/common/python/dns_query.py ${BIND} ${PORT} bupt 008.cn baidu.com test0 test1"
  python3 "$PY/dns_query.py" "$BIND" "$PORT" bupt 008.cn baidu.com test0 test1 2>&1 | expand_tabs || true
  echo ""
