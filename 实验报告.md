...
测试脚本：`scripts/test_dns.sh`（需 root：`sudo sh scripts/test_dns.sh`）。

以下为 **WSL2 + gcc** 环境下按配置 `dnsrelay.txt` 的 **预期输出要点**；提交前请运行脚本并将终端 **截图** 替换本节代码块。

**用例 1 — bupt 本地解析（预期 Address: 123.127.134.10）：**
...
**用例 3 — 008.cn 拦截（预期 NXDOMAIN / can't find）：**
...
**结论**：配置 `0.0.0.0 008.cn` 生效，拦截返回 RCODE=3。

**用例 4 — baidu.com 中继（预期返回公网 A 记录，非本地表 IP）：**
