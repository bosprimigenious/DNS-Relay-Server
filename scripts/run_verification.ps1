...
function Run-Nslookup([string]$Name, [string]$Qtype = "") {
    if ($Qtype -ne "") {
        ...
        $r = nslookup -port=$Port -type=$Qtype $Name $Bind 2>&1
    } else {
        $cmd = "nslookup -port=$Port $Name $Bind"
        $r = nslookup -port=$Port $Name $Bind 2>&1
    }
    Expand-Tabs "`$ $cmd"
...
function Start-Relay([string[]]$ExtraArgs) {
    $args = @("-b", $Bind, "-p", "$Port", "-f", $HostsFile, "-v") + $ExtraArgs
    return Start-Process -FilePath ".\dnsrelay.exe" -ArgumentList $args -PassThru `
...
Add-Log "`$ .\dnsrelay.exe -b $Bind -p $Port -f $HostsFile -v"
...
Add-Log "`$ python scripts\dns_query.py $Bind $Port bupt 008.cn baidu.com test0 test1"
Add-Log (Expand-Tabs (python "scripts\dns_query.py" $Bind $Port bupt 008.cn baidu.com test0 test1 2>&1 | Out-String))
Add-Log ""

Add-Log "Screenshot 11: dig bupt A +noall +answer +comments"
Add-Log (Expand-Tabs (python "scripts\dig_win.py" $Bind $Port bupt 2>&1 | Out-String))
Add-Log ""

Add-Log "Screenshot 12: dig 008.cn A +noall +answer +comments"
Add-Log (Expand-Tabs (python "scripts\dig_win.py" $Bind $Port 008.cn 2>&1 | Out-String))
Add-Log ""

Add-Log "Screenshot 13: dig baidu.com A +noall +answer +comments"
Add-Log (Expand-Tabs (python "scripts\dig_win.py" $Bind $Port baidu.com 2>&1 | Out-String))
Add-Log ""
...
Add-Log "`$ python scripts\dns_query.py $Bind $Port not-in-config-xyz123.com"
Add-Log (Expand-Tabs (python "scripts\dns_query.py" $Bind $Port not-in-config-xyz123.com 2>&1 | Out-String))
