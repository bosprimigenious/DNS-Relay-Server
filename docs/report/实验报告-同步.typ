...
实现层面，我们独立封装协议模块处理首部位域与 `htons`/`ntohs` 边界转换；在域名解码中引入指针跳转计数与偏移合法性校验；通过 ID 映射表记录转发会话（响应路由依赖调用栈中的 `client_addr`）；主循环以 `select()` 10ms 超时监听*单一*客户端 socket；上游中继使用*临时* UDP socket + `SO_RCVTIMEO=3s`，失败时返回 SERVFAIL（fix-B）。
...
事件驱动核心为 `select()`：将*客户端监听* fd 加入 `fd_set`，`timeval` 设为 10ms。返回 0 时无数据，进程在内核中短暂睡眠；返回大于 0 时处理可读事件。上游中继*不*占用监听 fd：每次中继在 `relay_to_upstream` 内创建临时 UDP socket 并设 `SO_RCVTIMEO=3s`，阻塞收包结束后关闭。
...
*同步版语义*：映射表用于审计与 ID 替换登记，*不*参与异步回包路由；`find_record_by_new_id` 仅在 `id_map.c` 单元逻辑中存在，主控层不调用。
...
该设计*优点*：逻辑线性、单函数可断点调试、与 RFC 1035 课设规模匹配。*局限*：上游 RTT 期间主循环无法 `select` 新查询——§4.5 dnsperf 压测已量化此瓶颈。
