// DNS 中继服务器 — 实验报告（同步中继版 · 分支 main）
// 北京邮电大学 (BUPT)
// 编译：make report

#set document(
  title: "DNS 中继服务器 · 实验报告（同步中继版）",
  author: "张恒基、尹浩铭、林旭东",
...
#align(center + top)[
  ...
  #v(0.5cm)
  #text(size: 17pt, fill: rgb("#475569"))[实验报告 · 同步中继版]
  #v(0.35cm)
  #text(size: 11pt, fill: rgb("#64748b"))[Git 分支 `main` · `relay_mode.h` = sync]
  #v(1.1cm)
  #block(
    width: 72%,
    inset: (x: 24pt, y: 20pt),
...
本报告专述 *同步上游中继* 实现（Git 分支 `main`）：`relay_to_upstream()` 在函数内阻塞 `recvfrom` 上游，主循环在处理一条中继查询期间无法接收新客户端报文。与通用递归 DNS（如 BIND、Unbound）不同，本课题将功能收敛为「策略驱动的转发网关」：依据 `dnsrelay.txt` 执行本地拦截、本地解析或上游转发；已实现 TTL 正向缓存与 CLI（`-b/-p/-s/-f/-c/-v`），不处理 TCP 53 fallback，主要面向 A 记录（QTYPE=1）的 UDP 查询。

实现层面，我们独立封装协议模块处理首部位域与 `htons`/`ntohs` 边界转换；在域名解码中引入指针跳转计数与偏移合法性校验；通过 ID 映射表记录转发会话（响应路由依赖调用栈中的 `client_addr`）；主循环以 `select()` 10ms 超时监听*单一*客户端 socket；上游中继使用*临时* UDP socket + `SO_RCVTIMEO=3s`，失败时返回 SERVFAIL（fix-B）。*异步双 socket 实现见仓库 `relay-async` 分支及《实验报告-异步》。*
...
*关键词*：DNS · UDP · RFC 1035 · 同步中继 · select · TTL 缓存
...
*第四，同步中继下的 ID 与会话管理。* UDP 无连接，Transaction ID 是客户端匹配响应的唯一线索。同步模型在 `relay_to_upstream` 内分配 new_id、`add_record` 后立即在同一函数阻塞等待上游；响应回客户端时还原 ID，路由信息来自函数参数 `client_addr`，*不*经 `find_record_by_new_id` 回调。ID 表仍用于记录与 5 秒老化，表满时触发 fix-B SERVFAIL。
...
#diagtext(
  ...
  [
    ...
    单线程避免对 `g_config`、`id_map`、`g_cache` 加锁；*设计取舍*是同步 `relay_to_upstream` 阻塞主循环——适合课设单用户演示，高并发见 §4.5 压测分析。启动链：`socket` → `bind` → `config_load` → `dns_cache_init` → `select` 主循环。
  ],
)
...
事件驱动核心为 `select()`：将*客户端监听* fd 加入 `fd_set`，`timeval` 设为 10ms。返回 0 时无数据，进程在内核中短暂睡眠；返回大于 0 时处理可读事件。上游中继*不*占用监听 fd：每次中继在 `relay_to_upstream` 内创建临时 UDP socket 并设 `SO_RCVTIMEO=3s`，阻塞收包结束后关闭——这是*同步版*与 `relay-async` 双持久 socket 模型的根本差异。
...
*ID 映射层*（`id_map.h` / `id_map.c`）在同步模型下记录 `(original_id, new_id, client)` 并支持 5 秒老化；`find_record_by_new_id` 已实现但*同步路径不用于回包*——响应在 `relay_to_upstream` 栈内直接 `sendto(client_addr)`。表满或 `add_record` 失败时 `relay_to_upstream` 返回 −1，主循环构造 SERVFAIL。
...
本组 ID 映射表固定容量 1024 条（`ID_MAP_SIZE`），采用环形槽位分配。每条记录含 `original_id`、`new_id`、`client_ip`、`client_port`、`created_at`。主循环每次收包前调用 `clear_timeout_records(now, 5)` 清理过期记录。`add_record` 失败时先强制清空再重试；仍失败则 `relay_to_upstream` 返回 −1 → fix-B SERVFAIL。

*同步版语义*：映射表用于审计与 ID 替换登记，*不*参与异步回包路由；`find_record_by_new_id` 仅在 `id_map.c` 单元逻辑中存在，主控层不调用。对比 `relay-async` 分支：`handle_upstream_response` 必须经 `find_record_by_new_id` 定位客户端后再 `sendto`。
...
`relay_to_upstream` 为*同步阻塞*实现（`include/relay_mode.h` 标识 `sync`）：创建临时 UDP socket → `SO_RCVTIMEO=3s` → 换 ID 并 `add_record` → `sendto` 上游 → *同函数内* `recvfrom` 阻塞 → 还原 ID → `sendto` 客户端 → 关闭临时 socket。成功响应写入 TTL 缓存。失败时主循环返回 SERVFAIL（fix-B）。
...
该设计*优点*：逻辑线性、单函数可断点调试、与 RFC 1035 课设规模匹配。*局限*：上游 RTT 期间主循环无法 `select` 新查询——§4.5 dnsperf 压测已量化此瓶颈；改进方案见 `relay-async` 分支《实验报告-异步》。
...
#keybox(
  [与异步版（`relay-async`）的对照],
  [
    *本报告（main / sync）*：单 client socket + 临时 upstream socket + 阻塞 `recvfrom`；课设默认交付与答辩演示。
    *《实验报告-异步》*：持久 `client_fd` + `upstream_fd`，`select` 双路监听，`handle_upstream_response` + ID 表路由；适合并发压测对比。
    两分支共享协议层、配置层、缓存与 CLI；差异集中在 `src/main.c` 中继调度。
  ],
)
